...
from app.exceptions import FileOperationException, ServiceException
...
class StoryDAO:
    ...
    async def _copy_character(self, story_dir_path: Path, character_path: Path) -> int:
        ...
        char_files_copied = await self.fs_ops.copy_dir(
            src=character_path, dest=new_character_dir, create_if_not_exists=True
        )
        return char_files_copied

    async def _create_story_meta(
        self, story_dir_path: Path, story_meta: MetaData
    ) -> None:
    ...
    async def create_story(self, character_path: Path, story_meta: MetaData) -> str:
        ...
        try:
            ...
            char_files_copied = await self._copy_character(
                new_story_dir, character_path
            )
            logger.info(
                "Copied %d character files for story %s",
                char_files_copied,
                new_story_id,
            )
            ...
            logger.info(
                "Story %s created successfully at %s", new_story_id, new_story_dir
            )
            return new_story_id

        except FileOperationException as e:
            logger.error(
                "File operation failed during story creation %s: %s",
                new_story_id,
                e.message,
            )
    ...
    async def _rollback_story_creation(
        self, story_dir_path: Path, story_id: str
    ) -> None:
        """Handle rollback of partially created story directory."""
        try:
            logger.info(
                "Rolling back: deleting partially created story directory %s",
                story_dir_path,
            )
            ...
            try:
                ...
                if metadata_file.exists():
                    ...
                    if isinstance(meta_data, dict) and "title" in meta_data:
                        ...
                        logger.warning(
                            f"Story {story_id}: meta.yaml exists but missing 'title' field"
                        )
                        title = "Untitled Story"
                else:
                    logger.debug(
                        f"Story {story_id}: meta.yaml not found, using default title"
                    )
                    title = "Untitled Story"
                ...
                logger.error(
                    "Failed to process story %s: %s", story_id, e, exc_info=True
                )
