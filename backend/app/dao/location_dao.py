...
class LocationDAO:
    ...
    def __init__(
        ...
        self.locations_dir = Path(
            locations_dir or path_manager.get_locations_base_dir()
        )
    ...
    async def get_location(self, id: str) -> Location:
        ...
        if not location_file.exists():
            raise FileNotFoundError(
                f"Location with id {id} not found at {location_file}"
            )
        ...
        if not isinstance(location_data, dict):
            raise ValueError(
                f"Invalid location data format in {location_file}, expected dict but got {type(location_data)}"
            )
    ...
    async def get_locations(self) -> list[Location]:
        ...
        async def load_location(location_file: Path) -> Location | None:
            ...
            try:
                ...
                if isinstance(location_data, dict):
                    ...
                    print(
                        f"Warning: Invalid location data format in {location_file}, expected dict but got {type(location_data)}"
                    )
                    return None
        ...
        location_results = await asyncio.gather(
            *[load_location(file) for file in location_files], return_exceptions=False
        )
