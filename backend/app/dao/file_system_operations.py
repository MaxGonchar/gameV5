...
class FileSystemOperations:
    ...
    async def create_dir(
        self, path: Path, parents: bool = True, exist_ok: bool = False
    ) -> None:
        """
    ...
    async def copy_dir(
        self, src: Path, dest: Path, create_if_not_exists: bool = True
    ) -> int:
        """
        ...
        if not src.is_dir():
            raise FileOperationException(
                f"Source path is not a directory: {src}", details={"src_path": str(src)}
            )
        ...
        try:
            ...
            logger.info(
                "Successfully copied %d files from %s to %s", files_copied, src, dest
            )
            return files_copied
    ...
    async def create_yaml_file(
        self, path: Path, content: Union[Dict[str, Any], List[Dict[str, Any]]]
    ) -> None:
        """
    ...
    async def delete_dir(self, path: Path) -> None:
        ...
        try:
            if not path.exists():
                raise FileOperationException(
                    f"Directory does not exist: {path}", details={"path": str(path)}
                )

            if not path.is_dir():
                raise FileOperationException(
                    f"Path is not a directory: {path}", details={"path": str(path)}
                )
