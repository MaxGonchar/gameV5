            ...
            "dynamic_configs": [
                {
                    ...
                    "decay_level": 0.9,
                    "dynamic_directives": {
                        "positive": {"axis": [0.1, 0.2, 0.3]},  # Simplified 3D vector
                        "negative": {
                    ...
                    "levels": [
                        {
                            "range": [0.0, 1.0],
                            "character_impacts": [{"title": "mood", "value": "happy"}],
                        },
                        {
                            "range": [-1.0, 0.0],
                            "character_impacts": [{"title": "mood", "value": "sad"}],
                        },
                    ],
                }
            ],
...
class TestCharacterInitialization(TestCharacter):
    ...
    def test_init_with_dynamic_configs(self, character_data_with_dynamics):
        ...
        assert (
            character.dynamic_configs == character_data_with_dynamics["dynamic_configs"]
        )
...
class TestCharacterDynamicBehavior(TestCharacter):
    ...
    def test_calculate_impact_positive_similarity(self):
        ...
        impact = character._calculate_impact(
            positive_axis, negative_axis, message_vector
        )
        # Expected: pos_sim = 1.0 (perfect alignment), neg_sim = 0.0 (orthogonal)
        # So impact = 1.0 - 0.0 = 1.0
        assert (
            abs(impact - 1.0) < 1e-10
    ...
    def test_correct_property_updates_character(self):
        ...
        mood_properties = [
            p for p in character.character if p["title"].lower() == "mood"
        ]
    ...
    def test_update_dynamic_configs_according_to_message_embeddings(
        self, character_data_with_dynamics
    ):
        ...
        with patch.object(character, "_update_config") as mock_update:
            ...
            mock_update.assert_called_once_with(
                character.dynamic_configs[0], embeddings
            )
